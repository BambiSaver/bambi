...
$uploader->sizeLimit = 15728640; // 15 MB = 15 * 1024 * 1024 bytes;
...
if ($method == "POST") {
    ...
    if (isset($_GET["done"])) {
        $result = $uploader->combineChunks("img-data");
    }
    // Handles upload requests
    else {
        // Call handleUpload() with the name of the folder, relative to PHP's getcwd()
        $result = $uploader->handleUpload("img-data");
...
// for delete file requests
    ...
    $result = $uploader->handleDelete("img-data");
    echo json_encode($result);
}
else {
