...
class UploadHandler {
    ...
    protected function getUniqueTargetPath($uploadDirectory, $filename)
    {
        ...
        $idFileName = $uploadDirectory . DIRECTORY_SEPARATOR . ".next.txt";
        $idFile = NULL;
        $id = 1;

        if (is_readable($idFileName)) {
            // get next id
        ...
        $newBase = sprintf("%'.05d", $id);
        $newFileName = $uploadDirectory . DIRECTORY_SEPARATOR . $newBase . $ext;

        while (file_exists($newFileName) && $id < 100000) {
            ++$id;
            $newBase = sprintf("%'.05d", $id);
            $newFileName = $uploadDirectory . DIRECTORY_SEPARATOR . $newBase . $ext;
        }
